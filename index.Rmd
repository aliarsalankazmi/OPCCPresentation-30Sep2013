...
```{r loadTm, echo=FALSE, results="hide", message=FALSE, warning=FALSE}
require(tm)
v1<- "vital issue more police must be on beat increase beat"
v2<- "police needs solve problem car parks taken over by bicycle please increase police beat"
v3<- "bicyclist proud"
corpus<- Corpus(VectorSource(c(v1,v2,v3)))
tdm1<- TermDocumentMatrix(corpus)
df1<- as.data.frame(inspect(tdm1))
names(df1)<- c("Document 1","Document 2","Document 3")
tdm2<- TermDocumentMatrix(corpus,control=list(weighting=function(x)weightTfIdf(x,normalize=FALSE)))
df2<- as.data.frame(inspect(tdm2))
names(df2)<- c("Document 1","Document 2","Document 3")
tdm3<- TermDocumentMatrix(corpus,control=list(weighting=function(x) weightSMART(x,spec="bnn")))
df3<- as.data.frame(inspect(tdm3))
names(df3)<- c("Document 1","Document 2","Document 3")
head(df1)
head(df2)
head(df3)
```
